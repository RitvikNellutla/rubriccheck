...
export const Header: React.FC<HeaderProps> = ({ showStartOver, onStartOver, result, isStrict, essayExplanation }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#fdfbf7] py-3 px-1 md:py-4 border-b border-stone-200 transition-all duration-300 flex items-center justify-between shadow-none">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg md:text-xl font-bold text-ink tracking-tight">RubricCheck</h1>
          {!showStartOver && (
             <p className="text-xs text-pencil hidden sm:block">Work checks made simple.</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        {result && (
          <>
             <button 
               onClick={handleCopy}
               className={`flex items-center gap-2 px-3 py-1.5 md:px-3 md:py-2 text-xs font-bold border rounded-lg transition-all shadow-sm active:scale-95 ${
                 isCopied 
             ...
             <div className="relative" ref={menuRef}>
                <button 
                   ...
                   className="flex items-center gap-2 px-3 py-1.5 md:px-3 md:py-2 text-xs font-bold text-stone-600 bg-white border border-stone-300 rounded-lg hover:bg-stone-50 transition-all shadow-sm active:scale-95"
                   title="Save Options"
                >
        ...
        {showStartOver && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-bold text-stone-600 bg-white border border-stone-300 rounded-lg hover:bg-stone-800 hover:text-white hover:border-stone-800 transition-all shadow-sm active:scale-95"
            title="Reset"
          >
