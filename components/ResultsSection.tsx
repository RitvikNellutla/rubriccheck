...
export const ResultsSection: React.FC<ResultsSectionProps> = ({ result, appState, onViewModeChange, onOverride }) => {
  ...
  useEffect(() => {
    ...
    const startNudgeTimer = () => {
      ...
      nudgeTimerRef.current = window.setTimeout(() => {
        if (window.scrollY <= 250 && !nudgeDismissed) {
  ...
  return (
    <div className="pt-4 animate-fade-in relative scroll-mt-[100px]">
      ...
      <div className={`fixed right-8 top-[130px] z-[100] transition-all duration-700 pointer-events-none ${showScrollNudge ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0'}`}>
        <div className="bg-[#1c1a19] text-white w-60 p-7 rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.5)] border border-stone-800 flex flex-col items-start gap-4 pointer-events-auto group relative animate-float">
          ...
          >
            <svg className="w-4 h-4 text-stone-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
          </button>
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-yellow-500/10 rounded-xl flex items-center justify-center text-yellow-500 border border-yellow-500/20 shrink-0">
               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M7 13l5 5 5-5M7 6l5 5 5-5"/></svg>
            </div>
          ...
          <div className="space-y-2.5">
             ...
             <p className="text-[10px] text-stone-500 font-bold leading-relaxed uppercase tracking-widest">
               Every part of your work is broken down below.
             </p>
          </div>
        </div>
        <style>{`
      ...
      {/* REFINED SUMMARY BOX - "THE BLACK BOX" (Slightly shorter version with subtle shadow to prevent bleed) */}
      <div className="bg-[#1c1a19] text-stone-50 rounded-[3rem] p-7 md:p-10 mb-10 border border-stone-800 overflow-visible relative group/summary shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-[1.6] bg-[#110f0e]/60 border border-stone-800 rounded-[2.2rem] p-8 flex flex-col items-center justify-center relative overflow-hidden group min-h-[280px]">
            ...
            <div className={`text-[100px] md:text-[130px] font-black leading-none tabular-nums transition-colors duration-1000 tracking-tighter ${displayScore >= 70 ? 'text-[#4ade80]' : displayScore >= 40 ? 'text-[#facc15]' : 'text-[#f87171]'}`}>
              {displayScore}
            </div>
            {adjustedMetrics.score !== summary.score && (
              <div className="mt-3 text-[9px] font-black uppercase text-stone-600 tracking-widest text-center">this is the score you would have received</div>
            )}
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <button key={s.label} onClick={() => scrollToStatus(s.label === 'good' ? 'met' : s.label as any)} className={`${s.bg} rounded-[1.8rem] p-6 flex flex-col items-center border border-stone-800 transition-all hover:border-stone-600 active:scale-95 group shadow-sm`}>
        ...
        <div className="border-t border-stone-800/60 pt-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {summary.top_fixes.map((fix, i) => (
                <div key={i} className="bg-[#110f0e] p-6 rounded-[1.8rem] border border-stone-800 flex gap-4 items-start group hover:border-stone-700 transition-colors shadow-sm">
                  ...
                  <div>
                    <p className="text-stone-100 font-bold text-[13px] mb-1 leading-tight">{fix.fix}</p>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex bg-stone-200/50 p-2 rounded-[1.25rem]">
            <button 
              ...
              className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl transition-all ${viewMode === 'list' ? 'bg-white text-stone-900 shadow-md' : 'text-stone-500 hover:text-stone-700'}`}
            >
              List View
            </button>
            <button 
              onClick={() => onViewModeChange('feedback')} 
              className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] rounded-xl transition-all ${viewMode === 'feedback' ? 'bg-white text-stone-900 shadow-md' : 'text-stone-500 hover:text-stone-700'}`}
            >
              Feedback Mode
            </button>
          </div>
      ...
      {viewMode === 'list' ? (
        ...
        <div id="split-container" className="flex flex-col lg:flex-row gap-8 items-start pb-4">
          <div className="w-full lg:sticky lg:top-[110px] max-h-[calc(100vh-12rem)] overflow-y-auto rounded-[3rem] border border-stone-200/60 custom-scrollbar shadow-none" style={{ width: window.innerWidth >= 1024 ? `${leftPanelWidth}%` : '100%' }}>
          ...
          <div onMouseDown={handleMouseDown} className="hidden lg:block w-2 h-[600px] cursor-col-resize hover:bg-stone-300 rounded-full transition-colors z-30 self-center"></div>
          
          <div className="flex-1 space-y-5" style={{ width: window.innerWidth >= 1024 ? `${100 - leftPanelWidth}%` : '100%' }}>
            {criteria.map((c, idx) => (
